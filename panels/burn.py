...
class Panel(ScreenPanel):
    ...
    def confirm_print(self, widget, filename):
        action = _("Wypalaj") if self._printer.extrudercount > 0 else _("Start")

        buttons = [
            {"name": _("Usuń"), "response": Gtk.ResponseType.REJECT, "style": 'dialog-error'},
            {"name": action, "response": Gtk.ResponseType.OK, "style": 'dialog-primary'},
            {"name": _("Anuluj"), "response": Gtk.ResponseType.CANCEL, "style": 'dialog-secondary'}
        ]
